export const contenido = [
  {
    id: 0,
    title: "VX DRONES REEL",
    details: `
    <dd>Música Original, Drone y Edición para <b>Vx Drones</b> (IG: @vx.drones)</dd>
    <dd>Producción: Vx Drones / Videomix</dd>
  `,
    image: require("../../assets/img/contenido/01-vx-reel.jpg"),
    video: "https://player.vimeo.com/video/336387398",
  },
  {
    id: 1,
    title: "40.000",
    details: `
    <dd>Sonido Directo en campaña RCP para la Asociación de Anestesia (AAARBA)</dd>
    <dd>Producción: Videomix (<a href="http://videomixargentina.com">http://videomixargentina.com</a>)</dd>
  `,
    image: require("../../assets/img/contenido/02-rcp-40mil.jpg"),
    video: "https://player.vimeo.com/video/371453162",
  },
  {
    id: 2,
    title: "VIDEOMIX COVID19",
    details: `
    <dd>MGrabación de canción adaptada y Drone</dd> 
    <dd>Realización: Gerardo Rissotti</dd>
    <dd>Producción: <b>Videomix</b> (<a href="http://videomixargentina.com">http://videomixargentina.com</a>)</dd>
  `,
    image: require("../../assets/img/contenido/03-covid-19.jpg"),
    video: "https://player.vimeo.com/video/453766618",
  },
  {
    id: 3,
    title: "SUPERDOMO LA RIOJA",
    details: `
    <dd>Música Original, Drone y Edición para <b>Sergio Guerrera, Club Led</b> (<a href="http://www.clubled.com.ar/#main">http://www.clubled.com.ar/#main</a>)</dd>
    <dd>Producción: Vx Drones / Jacarandá Polvo</dd>
  `,
    image: require("../../assets/img/contenido/04-superdomo.jpg"),
    video: "https://player.vimeo.com/video/336377644",
  },
  {
    id: 4,
    title: "GNOCCHI DEL TIGRE",
    details: `
    <dd>Música Original en Recetas de Cocina para <b>Elvira Me Gusta</b> (<a href="https://vimeo.com/elviramegusta">https://vimeo.com/elviramegusta</a>)</dd>
    <dd>Producción: Gerardo Rissotti</dd>
  `,
    image: require("../../assets/img/contenido/05-gnochi.jpg"),
    video: "https://player.vimeo.com/video/61352034",
  },
  {
    id: 5,
    title: "CANCIÓN DE LA REANIMACIÓN",
    details: `
    <dd>Grabación de canción y Edición de Video en Campaña RCP desde mi Cole para AAARBA</dd>
    <dd>Músicxs: Sofia Dellafiore + Facundo Rissotti + Coro de niñxs</dd>
    <dd>Producción: Jacarandá Polvo / Videomix</dd>
  `,
    image: require("../../assets/img/contenido/06-cancion-de-la-reanimacion.jpg"),
    video: "https://www.youtube.com/embed/d9CEC6Qoj_o",
  },
  {
    id: 6,
    title: "RCP OFICINA",
    details: `
    <dd>Sonido Directo en campaña RCP para la Asociación de Anestesia.</dd>
    <dd>Producción: Videomix (<a href="http://videomixargentina.com">http://videomixargentina.com</a>)</dd>
  `,
    image: require("../../assets/img/contenido/07-rcp-oficina.jpg"),
    video: "https://player.vimeo.com/video/295429904",
  },
  {
    id: 7,
    title: "OBRAS BA",
    details: `
    <dd>Música Original, Drone y Edición para <b>Vx Drones</b> (IG: @vx.drones)</dd>
    <dd>Producción: Vx Drones / Jacarandá Polvo</dd>
  `,
    image: require("../../assets/img/contenido/08-obras.jpg"),
    video: "https://player.vimeo.com/video/336376901",
  },
  {
    id: 8,
    title: "CAPRESE DE LA HUERTA",
    details: `
    <dd>Música Original en Recetas de Cocina para <b>Elvira Me Gusta</b> (<a href="https://vimeo.com/elviramegusta">https://vimeo.com/elviramegusta</a>)</dd>
    <dd>Producción: Gerardo Rissotti</dd>
  `,
    image: require("../../assets/img/contenido/09-caprese.jpg"),
    video: "https://player.vimeo.com/video/164428161",
  },
  {
    id: 9,
    title: "RCP FUTBOL ",
    details: `
    <dd>Sonido Directo y Drone en campaña RCP para la Asociación de Anestesia.</dd>
    <dd>Producción: <b>Videomix</b> (<a href="http://videomixargentina.com">http://videomixargentina.com</a>)</dd>
  `,
    image: require("../../assets/img/contenido/10-futbol.jpg"),
    video: "https://player.vimeo.com/video/285806014",
  },
];
